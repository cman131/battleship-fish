...
# Version 3.5 6/8/09
#     Modified for ease of use, checkMouseClick and additional objects.
#     Edits by Conor Wright
...
class Rectangle(_BBox):
    ...
    def checkMouseClick(self, click):
        p=self.getP1()
        p2=self.getP2()
        p1x=p.getX()
        p1y=p.getY()
        ...
        cx=click.getX()
        cy=click.getY()
        if cx>px and cx<p2x and cy>py and cy<p2y:
            return True
        else:
            return False
...
class Square(Rectangle):
    ...
    def clone(self):
        other = Square(self.getCenter(), self.radius)
        other.config = self.config.copy()
        return other
...
class Image(GraphicsObject):
    ...
    def getWidth(self):
        return self.pixmap.getWidth()

    def getHeight(self):
        return self.pixmap.getHeight()

    def save(self):
        win=GraphWin("Save As", 300, 300)
        e=Entry(Point(150, 100), 10)
        r=Rectangle(Point(50, 225), Point(100, 175))
        r2=Rectangle(Point(200, 225), Point(250, 175))
        t=Text(Point(75 ,200), "Save")
        t2=Text(Point(225, 200), "Cancel")
        e.draw(win)
        r.draw(win)
        r2.draw(win)
        t.draw(win)
        ...
        while not done:
            m=win.getMouse()
            if r.checkMouseClick(m):
                n=e.getText()
                name=n+".gif"
                self.pixmap.save(name)
                done=True
            elif r2.checkMouseClick(m):
                done=True
        e.undraw()
        r.undraw()
        r2.undraw()
